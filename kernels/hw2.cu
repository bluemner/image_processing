...
__global__ void apply_mask(const int segment_start, // start of segment 
        ...
        for(int j=segment_start; j< segment_size; j++){
             for(int k=0; k <  mask_size; k++){
                for(int l=0;l <  mask_size; l++){
                    ...
                    temp_0 += mask[l *mask_size + k] * original[l_shift*x_dimension+k_shift ];
                    temp_1 += mask[l *mask_size + k];
...
int main(int argc, char * argv[]){
    ...
    // Memory allocation
    T* mask = new T[mask_size*mask_size];
    ...
    */
    ...
    unsigned char * temp_stitch = new unsigned char[alt_x * y_dimension];
    for(int i =0; i<x_dimension; i++){// columns
        for(int j=0; j<y_dimension; j++){//rows
            temp_stitch[j*alt_x +i]=test_image[j*x_dimension+i];
            temp_stitch[j*alt_x +i+x_dimension+10]=fi[j*x_dimension+i];
            temp_stitch[j*alt_x +i+2*x_dimension+20]=gi[j*x_dimension+i];
        }
    }
    UWM::PGM().write("STITCH_SOURCE_BIF_GF.pgm",temp_stitch, alt_x, y_dimension);
    delete temp_stitch;
    // Clean up
    for(int i = 0; i < mask_size; i++){
       delete mask[i];
